...
#define CUDA_ASSERT(s)                                                                                                 \
    {                                                                                                                  \
        cudaError_t err = s;                                                                                           \
        if (err != cudaSuccess) {                                                                                      \
            throw std::runtime_error("CUDA command returned " + std::to_string((int)err) + ": " +                      \
...
void cuda_query()
{
    ...
    for (int dev = 0; dev < devicecount; ++dev) {
        ...
        cout << "Cores             : " << prop.multiProcessorCount << " MP x "
             << cores_per_mp((prop.major << 4) + prop.minor)
             << " cores/MP = " << prop.multiProcessorCount * cores_per_mp((prop.major << 4) + prop.minor) << " cores"
             << endl;
...
#define HASHCLASH_SHA1COMPRESS_ROUND1_STEPB(a, b, c, d, e, m)                                                          \
    e += rotate_left(a, 5) + sha1_f1(b, c, d) + 0x5A827999 + m;                                                        \
    b = rotate_left(b, 30);
#define HASHCLASH_SHA1COMPRESS_ROUND2_STEPB(a, b, c, d, e, m)                                                          \
    e += rotate_left(a, 5) + sha1_f2(b, c, d) + 0x6ED9EBA1 + m;                                                        \
    b = rotate_left(b, 30);
#define HASHCLASH_SHA1COMPRESS_ROUND3_STEPB(a, b, c, d, e, m)                                                          \
    e += rotate_left(a, 5) + sha1_f3(b, c, d) + 0x8F1BBCDC + m;                                                        \
    b = rotate_left(b, 30);
#define HASHCLASH_SHA1COMPRESS_ROUND4_STEPB(a, b, c, d, e, m)                                                          \
    e += rotate_left(a, 5) + sha1_f4(b, c, d) + 0xCA62C1D6 + m;                                                        \
    b = rotate_left(b, 30);
...
__global__ void sha1compress_test(uint32_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        uint32_t a = ihv[0][threadid];
        uint32_t b = ihv[1][threadid];
        uint32_t c = ihv[2][threadid];
        uint32_t d = ihv[3][threadid];
        uint32_t e = ihv[4][threadid];
...
void gpusha1benchmark()
{
    ...
    for (int blocks = 13; blocks <= MAXBLOCKS; blocks += 13) {
        for (int threads = 64; threads <= MAXTHREADSPERBLOCK; threads *= 2) {
            cout << "Starting kernel sha1compress_w_me in " << blocks << " block(s) * " << threads
                 << " threads: " << flush;
            sw.start();
    ...
    for (int blocks = 13; blocks <= MAXBLOCKS; blocks += 13) {
        for (int threads = 64; threads <= MAXTHREADSPERBLOCK; threads *= 2) {
            cout << "Starting kernel sha1compress_w_me2 in " << blocks << " block(s) * " << threads
                 << " threads: " << flush;
            sw.start();
