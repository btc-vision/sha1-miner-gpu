/*****
  Copyright (C) 2015 Pierre Karpman, INRIA France/Nanyang Technological University Singapore (-2016), CWI (2016/2017),
L'Universite Grenoble Alpes (2017-) (C) 2015 Marc Stevens, Centrum Wiskunde & Informatica (CWI), Amsterdam.
...
// - 1 block-wide
// - 2 gpu-wide
template <int fencetype>
__device__ void memoryfence()
{
    __threadfence();
}

template <>
__device__ void memoryfence<0>()
{}

template <>
__device__ void memoryfence<1>()
{
    __threadfence_block();
}
...
// cyclic_buffer_control_mask_t<N>, int fencetype = 2 > class cyclic_buffer_mask_t;
...
// template<size_t N, typename val_t = uint32_t, size_t val_cnt = 1, typename control_type =
// cyclic_buffer_control_cas_t<N>, int fencetype = 2 > class cyclic_buffer_cas_t;
...
#define ASSERT_COMPILE_TIME(b)                                                                                         \
    {                                                                                                                  \
        typename assert_compile_time_bool<(b)>::compile_time_bool_is_true ___assert = true;                            \
        if (!___assert) {                                                                                              \
            printf("boooh!");                                                                                          \
        }                                                                                                              \
    }
...
public:
    ...
    // assumes entire warp calls this function with identical inputs
    // if there is at least 1 element and read is safe return per-thread idx to read from or 0xEEEEEEEE if there was no
    // input for that thread, returns 0xFFFFFFFF otherwise (no work at all)
    template <typename safereadval_t>
...
template <size_t N, typename val_t = uint32_t, size_t val_cnt = 1,
          typename control_type = cyclic_buffer_control_mask_t<N>, int fencetype = 2>
...
public:
    ...
    __host__ __device__ inline void write(control_t &control, bool dowrite, val_t _val0, val_t _val1 = 0,
                                          val_t _val2 = 0, val_t _val3 = 0, val_t _val4 = 0, val_t _val5 = 0,
                                          val_t _val6 = 0, val_t _val7 = 0, val_t _val8 = 0, val_t _val9 = 0,
                                          val_t _val10 = 0, val_t _val11 = 0, val_t _val12 = 0, val_t _val13 = 0,
                                          val_t _val14 = 0, val_t _val15 = 0, val_t _val16 = 0, val_t _val17 = 0,
                                          val_t _val18 = 0, val_t _val19 = 0, val_t _val20 = 0, val_t _val21 = 0,
                                          val_t _val22 = 0, val_t _val23 = 0, val_t _val24 = 0, val_t _val25 = 0,
                                          val_t _val26 = 0, val_t _val27 = 0, val_t _val28 = 0, val_t _val29 = 0,
...
public:
    ...
    // variant partial warp read
    // assumes entire warp calls this function with identical inputs
    // if read is safe return per-thread idx to read from, 0xEEEEEEEE for no thread-specific input, or returns
    // 0xFFFFFFFF otherwise (no work at all)
    ...
    __host__ inline void host_write_finished() { ++written_idx; }
...
template <size_t N, typename val_t = uint32_t, size_t val_cnt = 1,
          typename control_type = cyclic_buffer_control_cas_t<N>, int fencetype = 2>
...
public:
    ...
    // called by entire warp
    __host__ __device__ inline void reset(control_t &control) { control.reset(); }
